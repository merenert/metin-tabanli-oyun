...
def test_equipment():
    ...
    karakter1.ekipman.kusan(esyalar["iron sword"].item_id, slot=OyuncuSlotu.SAG_KOL)
    karakter2.ekipman.kusan(esyalar["iron sword"].item_id, slot=None)
    assert karakter1.ekipman.kusanilan[OyuncuSlotu.SAG_KOL].item_id == esyalar["iron sword"].item_id
    assert karakter2.ekipman.kusanilan[OyuncuSlotu.SAG_KOL].item_id == esyalar["iron sword"].item_id
    savegame.save(karakter1, "karakter1.json")
    savegame.save(karakter2, "karakter2.json")
...
def test_combat():
    ...
    for _ in range(4):
        ...
        hasar = combat_system.saldir(aktif_karakter.stats, hedef.stats, OyuncuSlotu.SAG_KOL)
        print(f"{aktif_karakter.isim} saldırdı, verilen hasar: {hasar}, {hedef.isim} kalan can: {hedef.stats.can}")
...
def test_map_system():
    ...
    karakter1.konum.bulundugu_bolge = "orman_koyu"
    print(f"Başlangıç: {karakter1.konum.bulundugu_bolge}")
    karakter1.konum.bolge_degistir("orman", regions)
    print(f"Yeni konum: {karakter1.konum.bulundugu_bolge}")
    savegame.save(karakter1, "karakter1.json")
    savegame.save(karakter2, "karakter2.json")
